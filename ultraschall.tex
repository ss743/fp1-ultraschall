...
\gra{Mehrspalt}{Fraunhoferbeugungsmuster eines Gitters}
...
dabei gibt $\lambda$ die Wellenlänge des Lichts und $\Delta \lambda$ den Wellenlängenabstand an, bei dwelchem eine andere Wellenlänge von $\lambda$ bei der Beugung noch unterscheidbar ist. REF
...
\begin{align}
g(x) = \sum_{j = 0}^{\infty} \pm \sqrt{I_j}\cos\left( \frac{2 \pi j x}{K}\right) 
\end{align}
